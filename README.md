...
- **Автоматическое удаление тишины** - VAD (Voice Activity Detection) фильтрует паузы
...
## Удаление тишины (Remove Silence)

Скрипт автоматически использует VAD (Voice Activity Detection) для:
- Обнаружения пауз в речи (минимум 500ms тишины)
- Пропуска тихих моментов
- Создания субтитров только для речи

Это означает, что тихие моменты НЕ будут иметь субтитров, что удобно для дальнейшей обработки в DaVinci Resolve с функцией "Remove Silence".
